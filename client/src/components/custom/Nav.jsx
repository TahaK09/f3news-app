  ...
  return (
    <>
      <div className="w-full h-auto">
        {showPromo && (
          <div className="hidden lg:flex w-full min-h-20 py-2.5 font-medium text-sm text-white flex-row justify-between px-40 bg-gray-800 fixed top-0 left-0 z-40">
        ...
        <nav
          className={`z-50 fixed ${
            showPromo ? "lg:top-[80px]" : "lg:top-0"
          } left-0 right-0 flex items-center top-0 justify-between px-4 sm:px-6 md:px-16 lg:px-22 xl:px-32 py-3 border-b border-gray-300 bg-white transition-all`}
        >
          {/* Logo */}
            ...
            {navLink.map((item, index) => (
              <div key={index} className="inline font-semibold">
                ...
                {index < navLink.length - 1 && (
                  <span className="text-[#c7c7c7] font-light mx-3">|</span>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="hidden sm:flex flex-row gap-2">
            <button className="flex items-center gap-1 cursor-pointer p-2 rounded-full hover:bg-primary-dull transition bg-[#e2e2e2]">
              <Search className="w-4 h-4 text-black" />
            </button>
            <button className="flex items-center gap-1 px-4 py-1 hover:bg-primary-dull transition text-white text-sm bg-black rounded-lg">
          ...
          <div
            className={`${
              open ? "flex" : "hidden"
            } absolute top-full left-0 w-full bg-white font-semibold text-[#202020] shadow-md py-4 flex-col items-start gap-4 px-5 text-base sm:hidden`}
          >
            {navLink.map((item, i) => (
              <NavLink key={i} to={item.link} onClick={() => setOpen(false)}>
                {item.name}
              </NavLink>
            ))}
      ...
      <div className="flex lg:hidden w-full min-h-10 py-2.5 font-medium text-sm text-white flex-row justify-center gap-3 px-40 bg-gray-800 fixed top-16 left-0 z-40">
        <a
          href="https://www.facebook.com/f3news"
          className="gap-1.5 flex flex-row font-light text-md items-center justify-center"
        >
        ...
        <a
          href="https://www.medium.com/f3news"
          className="gap-1.5 flex flex-row font-light text-md items-center justify-center"
        >
          {medium_icon}
        </a>
